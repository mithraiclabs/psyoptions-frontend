...
const { DexInstructions, Market, MARKETS, TokenInstructions } = Serum

const programIds = (networkName) => {
  switch (networkName) {
    case 'Mainnet':
      return {
        dexProgramPubkey: MARKETS.find(({ deprecated }) => !deprecated)
          .programId,
        tokenProgramPubkey: new PublicKey(
          'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA'
        ),
      }
    case 'Devnet':
      return {
        dexProgramPubkey: new PublicKey(
    ...
    case 'localhost':
      const serumDexKeyBuffer = fs.readFileSync(
        ScriptHelpers.serumDexProgramKeypair
      )
