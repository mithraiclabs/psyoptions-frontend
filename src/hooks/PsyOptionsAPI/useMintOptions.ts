...
export const useMintOptions = (): ((
  ...
  return useCallback(
    async (option, size) => {
      ...
      const warpSolTransaction = new Transaction();
      ...
      if (option.underlyingAssetMint.toString() === WRAPPED_SOL_ADDRESS) {
        ...
        warpSolTransaction.add(wrapSolTx);
        signers.push(wrappedSolAccount);
        _underlyingAssetSourceKey = wrappedSolAccount.publicKey;
      }

      if (warpSolTransaction.instructions.length) {
        // create associated token accounts or wrapped Solana account if there are any instructions
        if (signers.length) {
          warpSolTransaction.feePayer = wallet.publicKey;
          warpSolTransaction.recentBlockhash = (
            await program.provider.connection.getRecentBlockhash('max')
          ).blockhash;
          warpSolTransaction.partialSign(...signers);
        }
        await wallet.signTransaction(warpSolTransaction);
        await sendSignedTransaction({
          signedTransaction: warpSolTransaction,
          connection: program.provider.connection,
          sendingMessage: 'Creating Wrapped Sol',
          successMessage: 'Successfully Wrapped Sol',
        });
      }
      ...
      const createAssociatedAcctstransaction = new Transaction();
      try {
        ...
        createAssociatedAcctstransaction.add(ix);
      }
      try {
      ...
      if (createAssociatedAcctstransaction.instructions.length) {
        try {
          pushNotification({
            severity: NotificationSeverity.INFO,
            message: 'Processing: Creating associated token accounts',
          });
          await program.provider.send(createAssociatedAcctstransaction);
          pushNotification({
            severity: NotificationSeverity.SUCCESS,
            message: 'Processing: Associated token accounts created!',
          });
        } catch (err) {
      ...
      if (option.underlyingAssetMint.toString() === WRAPPED_SOL_ADDRESS) {
        ...
        await program.provider.send(closeTransaction);
      }
    },
    [
      program,
